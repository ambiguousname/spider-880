#include "part1.h"
namespace MurderBlogPart1HTMLWindowNamespace {
std::shared_ptr<Text> text_3 = std::make_shared<Text>(
	"Anyone here see A Nightmare on Elm Street? Friday the 13th? ",
	std::vector<std::shared_ptr<HTMLNode>>{
	},
	std::unordered_map<std::string, std::string>{
	}
);
std::shared_ptr<P> p_3 = std::make_shared<P>(
	"",
	std::vector<std::shared_ptr<HTMLNode>>{
		text_3,
	},
	std::unordered_map<std::string, std::string>{
	}
);
std::shared_ptr<Text> text_5 = std::make_shared<Text>(
	"What about Silent Night, Deadly Night? Killer Workout? No? Or E.T., The Extra-Terrestrial?",
	std::vector<std::shared_ptr<HTMLNode>>{
	},
	std::unordered_map<std::string, std::string>{
	}
);
std::shared_ptr<P> p_5 = std::make_shared<P>(
	"",
	std::vector<std::shared_ptr<HTMLNode>>{
		text_5,
	},
	std::unordered_map<std::string, std::string>{
	}
);
std::shared_ptr<Text> text_7 = std::make_shared<Text>(
	"I'm not kidding around. 15 minutes after credits roll, that ",
	std::vector<std::shared_ptr<HTMLNode>>{
	},
	std::unordered_map<std::string, std::string>{
	}
);
std::shared_ptr<Text> text_9 = std::make_shared<Text>(
	"SPNPOLICY200",
	std::vector<std::shared_ptr<HTMLNode>>{
	},
	std::unordered_map<std::string, std::string>{
		{"href", "spider/policies"},
	}
);
std::shared_ptr<A> a_9 = std::make_shared<A>(
	"",
	std::vector<std::shared_ptr<HTMLNode>>{
		text_9,
	},
	std::unordered_map<std::string, std::string>{
		{"href", "spider/policies"},
	}
);
std::shared_ptr<Text> text_10 = std::make_shared<Text>(
	" turns the ship around and sticks his finger up the ",
	std::vector<std::shared_ptr<HTMLNode>>{
	},
	std::unordered_map<std::string, std::string>{
	}
);
std::shared_ptr<Text> text_12 = std::make_shared<Text>(
	"SPNPOLICY200",
	std::vector<std::shared_ptr<HTMLNode>>{
	},
	std::unordered_map<std::string, std::string>{
		{"href", "spider/policies"},
	}
);
std::shared_ptr<A> a_12 = std::make_shared<A>(
	"",
	std::vector<std::shared_ptr<HTMLNode>>{
		text_12,
	},
	std::unordered_map<std::string, std::string>{
		{"href", "spider/policies"},
	}
);
std::shared_ptr<Text> text_13 = std::make_shared<Text>(
	" of those kids. Or anyone too stuck-up to appreciate the childlike wonder and ",
	std::vector<std::shared_ptr<HTMLNode>>{
	},
	std::unordered_map<std::string, std::string>{
	}
);
std::shared_ptr<Text> text_15 = std::make_shared<Text>(
	"SPNPOLICY100",
	std::vector<std::shared_ptr<HTMLNode>>{
	},
	std::unordered_map<std::string, std::string>{
		{"href", "spider/policies"},
	}
);
std::shared_ptr<A> a_15 = std::make_shared<A>(
	"",
	std::vector<std::shared_ptr<HTMLNode>>{
		text_15,
	},
	std::unordered_map<std::string, std::string>{
		{"href", "spider/policies"},
	}
);
std::shared_ptr<Text> text_16 = std::make_shared<Text>(
	"-spewing goodness of Steven Spielberg's leftover ",
	std::vector<std::shared_ptr<HTMLNode>>{
	},
	std::unordered_map<std::string, std::string>{
	}
);
std::shared_ptr<Text> text_18 = std::make_shared<Text>(
	"SPNPOLICY200",
	std::vector<std::shared_ptr<HTMLNode>>{
	},
	std::unordered_map<std::string, std::string>{
		{"href", "spider/policies"},
	}
);
std::shared_ptr<A> a_18 = std::make_shared<A>(
	"",
	std::vector<std::shared_ptr<HTMLNode>>{
		text_18,
	},
	std::unordered_map<std::string, std::string>{
		{"href", "spider/policies"},
	}
);
std::shared_ptr<Text> text_19 = std::make_shared<Text>(
	". I say this not just for shock value, but to prepare you for some truly shocking news. Worse than that sneak-preview of E.T. 2 (coming to theaters this summer!)",
	std::vector<std::shared_ptr<HTMLNode>>{
	},
	std::unordered_map<std::string, std::string>{
	}
);
std::shared_ptr<P> p_7 = std::make_shared<P>(
	"",
	std::vector<std::shared_ptr<HTMLNode>>{
		text_19,
		a_18,
		text_16,
		a_15,
		text_13,
		a_12,
		text_10,
		a_9,
		text_7,
	},
	std::unordered_map<std::string, std::string>{
	}
);
std::shared_ptr<Text> text_21 = std::make_shared<Text>(
	"There's been another murder in Rorik's End. In other news, water is wet. And my ",
	std::vector<std::shared_ptr<HTMLNode>>{
	},
	std::unordered_map<std::string, std::string>{
	}
);
std::shared_ptr<Text> text_23 = std::make_shared<Text>(
	"SPNPOLICY100",
	std::vector<std::shared_ptr<HTMLNode>>{
	},
	std::unordered_map<std::string, std::string>{
		{"href", "spider/policies"},
	}
);
std::shared_ptr<A> a_23 = std::make_shared<A>(
	"",
	std::vector<std::shared_ptr<HTMLNode>>{
		text_23,
	},
	std::unordered_map<std::string, std::string>{
		{"href", "spider/policies"},
	}
);
std::shared_ptr<Text> text_24 = std::make_shared<Text>(
	" hasn't dropped yet.",
	std::vector<std::shared_ptr<HTMLNode>>{
	},
	std::unordered_map<std::string, std::string>{
	}
);
std::shared_ptr<P> p_21 = std::make_shared<P>(
	"",
	std::vector<std::shared_ptr<HTMLNode>>{
		text_24,
		a_23,
		text_21,
	},
	std::unordered_map<std::string, std::string>{
	}
);
std::shared_ptr<Text> text_26 = std::make_shared<Text>(
	"There's a pattern with these latest ones though. Police are calling him \"the slasher\" in a fit of creativity as rare as a unicorn ",
	std::vector<std::shared_ptr<HTMLNode>>{
	},
	std::unordered_map<std::string, std::string>{
	}
);
std::shared_ptr<Text> text_28 = std::make_shared<Text>(
	"SPNPOLICY200",
	std::vector<std::shared_ptr<HTMLNode>>{
	},
	std::unordered_map<std::string, std::string>{
		{"href", "spider/policies"},
	}
);
std::shared_ptr<A> a_28 = std::make_shared<A>(
	"",
	std::vector<std::shared_ptr<HTMLNode>>{
		text_28,
	},
	std::unordered_map<std::string, std::string>{
		{"href", "spider/policies"},
	}
);
std::shared_ptr<Text> text_29 = std::make_shared<Text>(
	" bricks in a nice salsa.",
	std::vector<std::shared_ptr<HTMLNode>>{
	},
	std::unordered_map<std::string, std::string>{
	}
);
std::shared_ptr<P> p_26 = std::make_shared<P>(
	"",
	std::vector<std::shared_ptr<HTMLNode>>{
		text_29,
		a_28,
		text_26,
	},
	std::unordered_map<std::string, std::string>{
	}
);
std::shared_ptr<Text> text_31 = std::make_shared<Text>(
	"Because the thing is, he slashes his victms. Around the neck. As I said, the inventiveness has really been kicked up a notch these days. Let's just hope he doesn't have any mommy issues, otherwise we'd really have to use our brains.",
	std::vector<std::shared_ptr<HTMLNode>>{
	},
	std::unordered_map<std::string, std::string>{
	}
);
std::shared_ptr<P> p_31 = std::make_shared<P>(
	"",
	std::vector<std::shared_ptr<HTMLNode>>{
		text_31,
	},
	std::unordered_map<std::string, std::string>{
	}
);
std::shared_ptr<Text> text_33 = std::make_shared<Text>(
	"Remember those films I was talking about earlier? I don't. The skinny of this sandwich that I'm slicing for you is, if you're going out at night. Go with someone. As long as they're going to kill you the second you turn your back.",
	std::vector<std::shared_ptr<HTMLNode>>{
	},
	std::unordered_map<std::string, std::string>{
	}
);
std::shared_ptr<P> p_33 = std::make_shared<P>(
	"",
	std::vector<std::shared_ptr<HTMLNode>>{
		text_33,
	},
	std::unordered_map<std::string, std::string>{
	}
);
std::shared_ptr<Body> body_2 = std::make_shared<Body>(
	"",
	std::vector<std::shared_ptr<HTMLNode>>{
		p_33,
		p_31,
		p_26,
		p_21,
		p_7,
		p_5,
		p_3,
	},
	std::unordered_map<std::string, std::string>{
	}
);
std::shared_ptr<HTMLNode> html_1 = std::make_shared<HTMLNode>(
	"",
	std::vector<std::shared_ptr<HTMLNode>>{
		body_2,
	},
	std::unordered_map<std::string, std::string>{
		{"title", "Murder in Rorik's End"},
	}
);
}
MurderBlogPart1HTMLWindow::MurderBlogPart1HTMLWindow(int x, int y, int w, int h) : HTMLWindow(MurderBlogPart1HTMLWindowNamespace::html_1, x, y, w, h) {
	linked_windows.insert({"spider/policies", SpiderPoliciesHTMLWindow::createWindow});
	linked_windows.insert({"spider/policies", SpiderPoliciesHTMLWindow::createWindow});
	linked_windows.insert({"spider/policies", SpiderPoliciesHTMLWindow::createWindow});
	linked_windows.insert({"spider/policies", SpiderPoliciesHTMLWindow::createWindow});
	linked_windows.insert({"spider/policies", SpiderPoliciesHTMLWindow::createWindow});
	linked_windows.insert({"spider/policies", SpiderPoliciesHTMLWindow::createWindow});


end();
}
