#include "messages.h"
namespace SpiderBbsMessagesHTMLWindowNamespace {
std::shared_ptr<Text> text_3 = std::make_shared<Text>(
	3,
	"DRYDOCK_ this wipe is ",
	std::vector<std::shared_ptr<HTMLNode>>{
	},
	std::unordered_map<std::string, std::string>{
		{"background-color", "12"},
		{"color", "7"},
	}
);
std::shared_ptr<Text> text_5 = std::make_shared<Text>(
	5,
	"SPNPOLICY200",
	std::vector<std::shared_ptr<HTMLNode>>{
	},
	std::unordered_map<std::string, std::string>{
		{"background-color", "12"},
		{"color", "4"},
		{"href", "spider/policies"},
	}
);
std::shared_ptr<A> a_5 = std::make_shared<A>(
	5,
	"",
	std::vector<std::shared_ptr<HTMLNode>>{
		text_5,
	},
	std::unordered_map<std::string, std::string>{
		{"background-color", "12"},
		{"color", "4"},
		{"href", "spider/policies"},
	}
);
std::shared_ptr<Text> text_6 = std::make_shared<Text>(
	6,
	" as ",
	std::vector<std::shared_ptr<HTMLNode>>{
	},
	std::unordered_map<std::string, std::string>{
		{"background-color", "12"},
		{"color", "7"},
	}
);
std::shared_ptr<Text> text_8 = std::make_shared<Text>(
	8,
	"SPNPOLICY200",
	std::vector<std::shared_ptr<HTMLNode>>{
	},
	std::unordered_map<std::string, std::string>{
		{"background-color", "12"},
		{"color", "4"},
		{"href", "spider/policies"},
	}
);
std::shared_ptr<A> a_8 = std::make_shared<A>(
	8,
	"",
	std::vector<std::shared_ptr<HTMLNode>>{
		text_8,
	},
	std::unordered_map<std::string, std::string>{
		{"background-color", "12"},
		{"color", "4"},
		{"href", "spider/policies"},
	}
);
std::shared_ptr<Text> text_9 = std::make_shared<Text>(
	9,
	". Not like engounge did anything wrong ",
	std::vector<std::shared_ptr<HTMLNode>>{
	},
	std::unordered_map<std::string, std::string>{
		{"background-color", "12"},
		{"color", "7"},
	}
);
std::shared_ptr<Text> text_11 = std::make_shared<Text>(
	11,
	"SPNPOLICY200",
	std::vector<std::shared_ptr<HTMLNode>>{
	},
	std::unordered_map<std::string, std::string>{
		{"background-color", "12"},
		{"color", "4"},
		{"href", "spider/policies"},
	}
);
std::shared_ptr<A> a_11 = std::make_shared<A>(
	11,
	"",
	std::vector<std::shared_ptr<HTMLNode>>{
		text_11,
	},
	std::unordered_map<std::string, std::string>{
		{"background-color", "12"},
		{"color", "4"},
		{"href", "spider/policies"},
	}
);
std::shared_ptr<P> p_3 = std::make_shared<P>(
	3,
	"",
	std::vector<std::shared_ptr<HTMLNode>>{
		a_11,
		text_9,
		a_8,
		text_6,
		a_5,
		text_3,
	},
	std::unordered_map<std::string, std::string>{
		{"background-color", "12"},
		{"color", "7"},
	}
);
std::shared_ptr<Text> text_13 = std::make_shared<Text>(
	13,
	"LSCHIFF Anyone looking for a new apartment? One of my roommates just moved out and I remember someone posting here about places to stay. ",
	std::vector<std::shared_ptr<HTMLNode>>{
	},
	std::unordered_map<std::string, std::string>{
		{"background-color", "12"},
		{"color", "7"},
	}
);
std::shared_ptr<P> p_13 = std::make_shared<P>(
	13,
	"",
	std::vector<std::shared_ptr<HTMLNode>>{
		text_13,
	},
	std::unordered_map<std::string, std::string>{
		{"background-color", "12"},
		{"color", "7"},
	}
);
std::shared_ptr<Text> text_15 = std::make_shared<Text>(
	15,
	"FORTYTWO Wow, college kid if I ever heard it. Get a room for yourself.",
	std::vector<std::shared_ptr<HTMLNode>>{
	},
	std::unordered_map<std::string, std::string>{
		{"background-color", "12"},
		{"color", "7"},
	}
);
std::shared_ptr<P> p_15 = std::make_shared<P>(
	15,
	"",
	std::vector<std::shared_ptr<HTMLNode>>{
		text_15,
	},
	std::unordered_map<std::string, std::string>{
		{"background-color", "12"},
		{"color", "7"},
	}
);
std::shared_ptr<Text> text_17 = std::make_shared<Text>(
	17,
	"LSCHIFF I wish.",
	std::vector<std::shared_ptr<HTMLNode>>{
	},
	std::unordered_map<std::string, std::string>{
		{"background-color", "12"},
		{"color", "7"},
	}
);
std::shared_ptr<P> p_17 = std::make_shared<P>(
	17,
	"",
	std::vector<std::shared_ptr<HTMLNode>>{
		text_17,
	},
	std::unordered_map<std::string, std::string>{
		{"background-color", "12"},
		{"color", "7"},
	}
);
std::shared_ptr<Text> text_19 = std::make_shared<Text>(
	19,
	"ADMIN Wrong channel for this discussion, please migrate.",
	std::vector<std::shared_ptr<HTMLNode>>{
	},
	std::unordered_map<std::string, std::string>{
		{"background-color", "12"},
		{"color", "7"},
	}
);
std::shared_ptr<P> p_19 = std::make_shared<P>(
	19,
	"",
	std::vector<std::shared_ptr<HTMLNode>>{
		text_19,
	},
	std::unordered_map<std::string, std::string>{
		{"background-color", "12"},
		{"color", "7"},
	}
);
std::shared_ptr<Text> text_21 = std::make_shared<Text>(
	21,
	"DRYDOCK_ Admins really taking to heart the \"lazy ",
	std::vector<std::shared_ptr<HTMLNode>>{
	},
	std::unordered_map<std::string, std::string>{
		{"background-color", "12"},
		{"color", "7"},
	}
);
std::shared_ptr<Text> text_23 = std::make_shared<Text>(
	23,
	"SPNPOLICY200",
	std::vector<std::shared_ptr<HTMLNode>>{
	},
	std::unordered_map<std::string, std::string>{
		{"background-color", "12"},
		{"color", "4"},
		{"href", "spider/policies"},
	}
);
std::shared_ptr<A> a_23 = std::make_shared<A>(
	23,
	"",
	std::vector<std::shared_ptr<HTMLNode>>{
		text_23,
	},
	std::unordered_map<std::string, std::string>{
		{"background-color", "12"},
		{"color", "4"},
		{"href", "spider/policies"},
	}
);
std::shared_ptr<Text> text_24 = std::make_shared<Text>(
	24,
	"\" policy of moderation.",
	std::vector<std::shared_ptr<HTMLNode>>{
	},
	std::unordered_map<std::string, std::string>{
		{"background-color", "12"},
		{"color", "7"},
	}
);
std::shared_ptr<P> p_21 = std::make_shared<P>(
	21,
	"",
	std::vector<std::shared_ptr<HTMLNode>>{
		text_24,
		a_23,
		text_21,
	},
	std::unordered_map<std::string, std::string>{
		{"background-color", "12"},
		{"color", "7"},
	}
);
std::shared_ptr<Text> text_26 = std::make_shared<Text>(
	26,
	"ADMIN Deleted. Please keep discussions SFW.",
	std::vector<std::shared_ptr<HTMLNode>>{
	},
	std::unordered_map<std::string, std::string>{
		{"background-color", "12"},
		{"color", "7"},
	}
);
std::shared_ptr<P> p_26 = std::make_shared<P>(
	26,
	"",
	std::vector<std::shared_ptr<HTMLNode>>{
		text_26,
	},
	std::unordered_map<std::string, std::string>{
		{"background-color", "12"},
		{"color", "7"},
	}
);
std::shared_ptr<Text> text_28 = std::make_shared<Text>(
	28,
	"SEDRICK okay fine. but for everyone else please be safe. they just found another slasher victim.",
	std::vector<std::shared_ptr<HTMLNode>>{
	},
	std::unordered_map<std::string, std::string>{
		{"background-color", "12"},
		{"color", "7"},
	}
);
std::shared_ptr<P> p_28 = std::make_shared<P>(
	28,
	"",
	std::vector<std::shared_ptr<HTMLNode>>{
		text_28,
	},
	std::unordered_map<std::string, std::string>{
		{"background-color", "12"},
		{"color", "7"},
	}
);
std::shared_ptr<Text> text_30 = std::make_shared<Text>(
	30,
	"NIETZSCHE is the only thing about this guy that he slashes the neck?",
	std::vector<std::shared_ptr<HTMLNode>>{
	},
	std::unordered_map<std::string, std::string>{
		{"background-color", "12"},
		{"color", "7"},
	}
);
std::shared_ptr<P> p_30 = std::make_shared<P>(
	30,
	"",
	std::vector<std::shared_ptr<HTMLNode>>{
		text_30,
	},
	std::unordered_map<std::string, std::string>{
		{"background-color", "12"},
		{"color", "7"},
	}
);
std::shared_ptr<Text> text_32 = std::make_shared<Text>(
	32,
	"ADMIN Stop. Just remember to stay safe all, and travel in groups.",
	std::vector<std::shared_ptr<HTMLNode>>{
	},
	std::unordered_map<std::string, std::string>{
		{"background-color", "12"},
		{"color", "7"},
	}
);
std::shared_ptr<P> p_32 = std::make_shared<P>(
	32,
	"",
	std::vector<std::shared_ptr<HTMLNode>>{
		text_32,
	},
	std::unordered_map<std::string, std::string>{
		{"background-color", "12"},
		{"color", "7"},
	}
);
std::shared_ptr<Text> text_34 = std::make_shared<Text>(
	34,
	"SEDRICK okay, leaving it",
	std::vector<std::shared_ptr<HTMLNode>>{
	},
	std::unordered_map<std::string, std::string>{
		{"background-color", "12"},
		{"color", "7"},
	}
);
std::shared_ptr<P> p_34 = std::make_shared<P>(
	34,
	"",
	std::vector<std::shared_ptr<HTMLNode>>{
		text_34,
	},
	std::unordered_map<std::string, std::string>{
		{"background-color", "12"},
		{"color", "7"},
	}
);
std::shared_ptr<Text> text_36 = std::make_shared<Text>(
	36,
	"5COOLBEANS Bump! It's expensive to live by yourself (I should know), anyone looking for roommates hit up lschiff.",
	std::vector<std::shared_ptr<HTMLNode>>{
	},
	std::unordered_map<std::string, std::string>{
		{"background-color", "12"},
		{"color", "7"},
	}
);
std::shared_ptr<P> p_36 = std::make_shared<P>(
	36,
	"",
	std::vector<std::shared_ptr<HTMLNode>>{
		text_36,
	},
	std::unordered_map<std::string, std::string>{
		{"background-color", "12"},
		{"color", "7"},
	}
);
std::shared_ptr<Text> text_38 = std::make_shared<Text>(
	38,
	"Powered by ",
	std::vector<std::shared_ptr<HTMLNode>>{
	},
	std::unordered_map<std::string, std::string>{
		{"background-color", "12"},
		{"color", "7"},
	}
);
std::shared_ptr<Text> text_40 = std::make_shared<Text>(
	40,
	"Mirror",
	std::vector<std::shared_ptr<HTMLNode>>{
	},
	std::unordered_map<std::string, std::string>{
		{"background-color", "12"},
		{"color", "4"},
		{"href", "mirror/index"},
	}
);
std::shared_ptr<A> a_40 = std::make_shared<A>(
	40,
	"",
	std::vector<std::shared_ptr<HTMLNode>>{
		text_40,
	},
	std::unordered_map<std::string, std::string>{
		{"background-color", "12"},
		{"color", "4"},
		{"href", "mirror/index"},
	}
);
std::shared_ptr<Text> text_41 = std::make_shared<Text>(
	41,
	".",
	std::vector<std::shared_ptr<HTMLNode>>{
	},
	std::unordered_map<std::string, std::string>{
		{"background-color", "12"},
		{"color", "7"},
	}
);
std::shared_ptr<P> p_38 = std::make_shared<P>(
	38,
	"",
	std::vector<std::shared_ptr<HTMLNode>>{
		text_41,
		a_40,
		text_38,
	},
	std::unordered_map<std::string, std::string>{
		{"background-color", "12"},
		{"color", "7"},
	}
);
std::shared_ptr<Body> body_2 = std::make_shared<Body>(
	2,
	"",
	std::vector<std::shared_ptr<HTMLNode>>{
		p_38,
		p_36,
		p_34,
		p_32,
		p_30,
		p_28,
		p_26,
		p_21,
		p_19,
		p_17,
		p_15,
		p_13,
		p_3,
	},
	std::unordered_map<std::string, std::string>{
		{"background-color", "12"},
		{"color", "7"},
	}
);
std::shared_ptr<HTMLNode> html_1 = std::make_shared<HTMLNode>(
	1,
	"",
	std::vector<std::shared_ptr<HTMLNode>>{
		body_2,
	},
	std::unordered_map<std::string, std::string>{
	}
);
}
SpiderBbsMessagesHTMLWindow::SpiderBbsMessagesHTMLWindow(int x, int y, int w, int h) : HTMLWindow(SpiderBbsMessagesHTMLWindowNamespace::html_1, x, y, w, h) {
	linked_windows.insert({"spider/policies", SpiderPoliciesHTMLWindow::createWindow});
	linked_windows.insert({"spider/policies", SpiderPoliciesHTMLWindow::createWindow});
	linked_windows.insert({"spider/policies", SpiderPoliciesHTMLWindow::createWindow});
	linked_windows.insert({"spider/policies", SpiderPoliciesHTMLWindow::createWindow});
	linked_windows.insert({"mirror/index", MirrorIndexHTMLWindow::createWindow});


end();
}
